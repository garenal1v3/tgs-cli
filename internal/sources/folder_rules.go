...
// peerKeyOf extracts a peerKey from a concrete InputPeer (User/Chat/Channel).
// Returns (zero, false) for InputPeerSelf or any unknown variant — callers
// that need to handle Self must do so before/after calling this helper.
func peerKeyOf(p tg.InputPeerClass) (peerKey, bool) {
	switch v := p.(type) {
...
func indexByPeer(dialogs []sourceWithPeer) map[peerKey]*sourceWithPeer {
	...
	for i := range dialogs {
		if k, ok := peerKeyOf(dialogs[i].Peer); ok {
			m[k] = &dialogs[i]
		}
	}
	return m
...
// Note: InputPeerSelf entries are silently skipped when selfID is unknown
// (== 0); callers must populate selfID for Saved-Messages exclusion to work.
