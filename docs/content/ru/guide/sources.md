...
В ответе появляются дополнительные поля, недоступные в `list`: `subscribed`, `description`, `creation_date`, `invite_link`. `creation_date` есть для каналов/супергрупп/групп; для пользователей не возвращается (Telegram не отдаёт дату регистрации).
