...
> **`total`** — это общее число диалогов, которое сообщает Telegram, **до** применения фильтра `--type`. Не делите `total` на длину массива `sources` для подсчёта страниц — для пагинации используйте `cursor`.
```
...
> **Ограничение числовых ID:** числовой ID работает только если этот peer уже есть в ваших диалогах или в локальном peer-кеше. Для инспектирования канала, на который вы не подписаны, используйте `@username` или `+телефон` — но не числовой ID.
