...
Каждая строка имеет вид `[дата] чат: текст` для постов в каналах или `[дата] чат | автор: текст`, когда у сообщения есть отдельный отправитель (группы, супергруппы). Счётчики реакций и просмотров добавляются в квадратных скобках, если присутствуют.
